...
document.addEventListener('DOMContentLoaded', function () {
    fetch('/data/anitlar.json')
        .then(response => response.json())
        .then(data => {
            const anitSayacDiv = document.getElementById('anit-sayac');
            data.forEach(anit => {
                const anitHtml = `
                    <div class="col-md-6 col-lg-4 mb-4">
                        <div class="card h-100">
                            <img src="${anit.resim}" class="card-img-top" alt="${anit.isim}">
                            <div class="card-body">
                                <h5 class="card-title">${anit.isim}</h5>
                                <p class="card-text">${anit.aciklama}</p>
                                <p class="card-text"><small class="text-muted">${anit.tarih}</small></p>
                            </div>
                        </div>
                    </div>
                `;
                anitSayacDiv.innerHTML += anitHtml;
            });
        })
